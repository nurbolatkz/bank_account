...
class AccountDatabasePandas(AccountDatabase):
    ...
    def _save(self, account: Account) -> None:
        ...
        self._objects = self._objects.append(new_row, ignore_index=True)
        self._objects.to_pickle("database.pk")
    ...
    def _delete(self, id_: UUID) -> None:
        if id_ not in list(self._objects["id"]):
            raise ObjectNotFound("Pandas error: object not found")

        self._objects.drop(self._objects[self._objects["id"] == id_].index, inplace=True)
